  ...
  return (
    <div className="relative overflow-x-auto dark:bg-gray-900 rounded-lg">
      <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 rounded-lg">
        ...
        <tbody>
          {jobs.map((elements, key) => (
            <React.Fragment key={elements.id}>
              <tr className="bg-white dark:bg-gray-900 rounded-lg">
                <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                  {elements.jobName}
                </td>
                <td className="px-6 py-4  min-w-[17rem] max-w-[17rem] overflow-hidden">
                  <div className="max-h-10 min-h-10  overflow-y-auto">{elements.requirements}</div>
                </td>
                <td className="px-6 py-4">{elements.enrollments}</td>
                <td className="px-6 py-4">
                  <Link
                    to={{
                      pathname: "/dashboard/enrollment",
                      state: { jobName: 'Software Engineer', requirements: 'React experience required' }
