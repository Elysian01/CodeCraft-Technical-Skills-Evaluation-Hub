...
function Jobform() {
  ...
  const createJobDTO = (jobName, jobDescription, roleType,company, requirements) => {
    return {
      jobName,
      jobDescription,
      roleType,
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const uploadData = createJobDTO(jobName, jobDescription, roleType,company, requirements);
      ...
      <div className="flex justify-center min-h-screen bg-gray-800 py-1">
        <div className="w-2/3 p-2">
          <form onSubmit={handleSubmit} className="bg-gray-900 rounded shadow-lg border border-gray-600 p-4 mb-6">
            <div className="mb-4">
              <label htmlFor="jobName" className="block text-sm text-white">Job Name:</label>
              <input
                id="jobName"
                value={jobName}
            ...
            <div className="mb-4">
              <label htmlFor="interviewerId" className="block text-sm text-white">Company Name:</label>
              <input
                type="number"
                id="interviewerId"
                value={company}
